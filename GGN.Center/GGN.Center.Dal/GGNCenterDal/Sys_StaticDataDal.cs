...
{
    public class Sys_StaticDataDal : BaseDal<Sys_StaticData>
    {
        #region 必备
        ...
        protected override DbSet<Sys_StaticData> DbSet
        {
            get
            {
                return activeContext.Sys_StaticData;
            }
        }
        /// <summary>
        /// 实体对象上下文
        ...
        /// <param name="Id"></param>
        /// <returns></returns>
        public  vSys_StaticData GetById(Guid Id)
        {
            var model= base.GetById(Id);
            var returnModel = CommonOperate.ConvertObj<vSys_StaticData>(model);
            return returnModel;
        }
        #endregion
        
        #region 检查mark是否重复
        public OperateStatus CheckMark(Sys_StaticData model)
        {
            OperateStatus op = new OperateStatus();
            try
            {
                var query = from temp in activeContext.Sys_StaticData
                            //where
                            //temp..Contains(model.)
                            select temp;
        ...
        public ListByPages<vSys_StaticData> QuickQuery(Sys_StaticDataQuickQueryParam queryParam)
        {
            var query = from temp in activeContext.Sys_StaticData
                        where
                          ...
                          || temp.Value.Contains(queryParam.KeyWords))
                        select new vSys_StaticData
                        {
                             Id = temp.Id,
        ...
        /// <returns></returns>
        public ListByPages<vSys_StaticData> Query(Sys_StaticDataQueryParam queryParam)
        {
            var query = from temp in activeContext.Sys_StaticData
                        where
                           (string.IsNullOrEmpty(queryParam.KeyWords)  

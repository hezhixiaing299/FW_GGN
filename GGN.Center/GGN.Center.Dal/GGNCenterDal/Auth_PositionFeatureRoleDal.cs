...
{
    public class Auth_PositionFeatureRoleDal : BaseDal<Auth_PositionFeatureRole>
    {
        #region 必备
        ...
        protected override DbSet<Auth_PositionFeatureRole> DbSet
        {
            get
            {
                return activeContext.Auth_PositionFeatureRole;
            }
        }
        /// <summary>
        /// 实体对象上下文
        ...
        #region 检查mark是否重复
        public OperateStatus CheckMark(Auth_PositionFeatureRole model)
        {
            OperateStatus op = new OperateStatus();
            try
            {
                var query = from temp in activeContext.Auth_PositionFeatureRole
                            //where
                            //temp..Contains(model.)
                            select temp;
        ...
        public ListByPages<vAuth_PositionFeatureRole> QuickQuery(Auth_PositionFeatureRoleQuickQueryParam queryParam)
        {
            var query = from temp in activeContext.Auth_PositionFeatureRole
                        where
                           (string.IsNullOrEmpty(queryParam.KeyWords)  )
                        select new vAuth_PositionFeatureRole
                        {
                             Id = temp.Id,
        ...
        /// <returns></returns>
        public ListByPages<vAuth_PositionFeatureRole> Query(Auth_PositionFeatureRoleQueryParam queryParam)
        {
            var query = from temp in activeContext.Auth_PositionFeatureRole
                        where
                           (string.IsNullOrEmpty(queryParam.KeyWords)  )
